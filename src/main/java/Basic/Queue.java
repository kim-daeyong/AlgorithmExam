package Basic;

public class Queue {

    // FIFO - 처음 들어간게 처음 나온다.

    private int max; //큐의 용량
    private  int front; // 첫번때 요소의 커서
    private  int rear; // 마지막 요서 커서
    private int num; //현재 데이터 수
    private int[] que ; //큐 본체;

    // 큐 비어있음
    public class EmptyIntQueueException extends RuntimeException{
        public EmptyIntQueueException(){

        }

    }// 큐가 가즉 참
    public class OverFlowIntQueueException extends RuntimeException{
        public OverFlowIntQueueException(){}

    }

    //생성자
    public Queue(int capacity){
        num = front = rear = 0; // 생성 시 큐는 비어있다.
        max = capacity;
        try{
            que = new int[max]; // 큐 본체용 배열을 생성
        }
        catch (OutOfMemoryError e){ // 생성할 수 없음

            max = 0;
        }
    }

    /*
    Queue - 큐를 관리하는 클래스
    front  첫번때 요소를 저장하는 인덱스맨
    rear 가장 맨 마지막에 넣은 요소나 하나위의 인덱스를 저장하는 필드
     */
    // 큐에 데이터를 인큐
    public int enque(int x) throws OverFlowIntQueueException{
        if (num >= max) throw new OverFlowIntQueueException();

        // 데이터를 인큐하고 rear와 num값을 1만큼 증가하면 인큐가 성공
        que[rear++] = x;
        num++;
        //하지만 rear값이 11 max와 같아지게 되는 문제가 발생
        // 그걸 방지하기위해 최대 용량 값과 같아지면 rear를 배열의 처음인 0으로 변경
        if(rear == max) rear = 0;

        return x;
    }

    // 디큐 메서드
    public int deque() throws EmptyIntQueueException{

        if (num <= 0) throw new EmptyIntQueueException(); // 큐가 비어있음

        // 데이터를 꺼내고 front값을 1만큼 증가. num을 1 감소
        int x = que[front++];
        num --;

        //인덱스 초과 문제 발생
        if(front == max)
            front = 0;

        return x;

    }


    // 큐에서 데이터를 피크 (프런트 데이터를 들여다봄)
    public int peek() throws EmptyIntQueueException {
        if (num <= 0)
            throw new EmptyIntQueueException();				// 큐가 비어 있음
        return que[front];
    }

    // 큐에서 x를 검색하여 인덱스(찾지 못하면 –1)를 반환
    public int indexOf(int x) {
        for (int i = 0; i < num; i++) {
            int idx = (i + front) % max;
            if (que[idx] == x)								// 검색 성공
                return idx;
        }
        return -1;											// 검색 실패
    }

    // 큐를 비움
    public void clear() {
        num = front = rear = 0;
    }

    // 큐의 용량을 반환
    public int capacity() {
        return max;
    }

    // 큐에 쌓여 있는 데이터 수를 반환
    public int size() {
        return num;
    }

    // 큐가 비어 있나요?
    public boolean isEmpty() {
        return num <= 0;
    }

    // 큐가 가득 찼나요?
    public boolean isFull() {
        return num >= max;
    }

    // 큐 안의 모든 데이터를 프런트 → 리어 순으로 출력
    public void dump() {
        if (num <= 0)
            System.out.println("큐가 비어 있습니다.");
        else {
            for (int i = 0; i < num; i++)
                System.out.print(que[(i + front) % max] + " ");

            System.out.println();
        }
    }


}
